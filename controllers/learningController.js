...
exports.scoreCard = async (req, res) => {
  try {
    ...
    // ✅ Step 7: Update Assigned averages using endDate
    try {
      const assignedList = await Assigned.find({
        endDate: userEndDate,
        classId: userClassId
...
//     // ✅ Step 1: Get first score per day — removed session filter
//     const rawScores = await TopicScore.aggregate([
//       {
//         $match: {
//           userId: new mongoose.Types.ObjectId(userId),
...
//     // Step 7: Update Assigned averages (filtered by classId + endDate)
//     try {
//       const today = new Date();
//       const allAssigned = await Assigned.find({ classId: userClassId }).lean();

//       const validAssigned = allAssigned.filter(assign => {
//         if (!assign.endDate) return true;
//         const [day, month, year] = assign.endDate.split('-');
//         const endDate = new Date(`${year}-${month}-${day}T23:59:59`);
//         return endDate >= today;
//       });

//       for (let assign of validAssigned) {
//         const update = {};
...
exports.genraliqAverage = async (req, res) => {
  try {
    ...
    // Fetch all practice and topic scores sorted by createdAt (earliest first)
    ...
    // Collect earliest practice for each date
    ...
    // Collect earliest topic for each date
    ...
    for (let [date, record] of dateWiseMap.entries()) {
      ...
      // Skip this date if neither score belongs to the requested learningId
      ...
      // Only include this date if either practice or topic (whichever exists) is of matching learningId
      if (
