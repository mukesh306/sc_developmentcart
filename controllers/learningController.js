...
exports.Dashboard = async (req, res) => {
  try {
    ...
    // --- Session-based Level Bonus Point ---
    let levelBonusPoint = 0;
    if (Array.isArray(user?.userLevelData)) {
      const levelData = user.userLevelData.find(
        item => item.level === level && item.session === session
      );
      levelBonusPoint = levelData?.levelBonusPoint || 0;
    }
