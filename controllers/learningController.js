  ...
  try {
    ...
    if (learningScores.length && topicScores.length) {
      const practiceDates = new Set(
        learningScores.map(s =>
          moment(s.scoreDate || s.createdAt).format('YYYY-MM-DD')
        )
      );
      const topicDates = new Set(
        topicScores.map(s =>
          moment(s.updatedAt || s.createdAt).format('YYYY-MM-DD')
        )
      );
    ...
    let assignedLearnings = await Assigned.find({ classId })
      .populate('learning')
      .populate('learning2')
      .populate('learning3')
      .populate('learning4')
    ...
    for (const item of assignedLearnings) {
      for (const field of ['learning', 'learning2', 'learning3', 'learning4']) {
        const lrn = item[field];
        if (lrn && lrn._id && !seen.has(lrn._id.toString())) {
          seen.add(lrn._id.toString());
          practice.push({ ...lrn, totalQuiz });
        }
      }
    }
    ...
    if (expData?.levelBonusPoint != null) {
      levelBonusPoint = Math.round(expData.levelBonusPoint);
    } else if (Array.isArray(user.userLevelData)) {
      const ld = user.userLevelData.find(l => l.level === user.level);
      if (ld?.levelBonusPoint != null) {
        levelBonusPoint = Math.round(ld.levelBonusPoint);
      }
    }

    
    let classInfo = await School.findById(classId).lean();
    if (!classInfo) classInfo = await College.findById(classId).lean();
