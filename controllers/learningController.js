...
exports.scoreCard = async (req, res) => {
  try {
    ...
    let startDate = fromDate ? moment(fromDate).startOf("day") : moment(user.updatedAt).startOf("day");
    let endDate = toDate ? moment(toDate).startOf("day") : today;
    ...
    // Aggregate without date filter → one score per day
    const rawScores = await TopicScore.aggregate([
      { $match: match },
      { $sort: { scoreDate: 1, createdAt: 1 } },
      {
    ...
    // Generate full result for all dates in range
    const fullResult = [];
    ...
    for (const entry of fullResult) {
      if (entry.score !== null && entry.learningId?._id) {
        const lid = entry.learningId._id.toString();
        const lname = entry.learningId.name || "Unknown";
        if (!learningScores[lid]) learningScores[lid] = { learningId: lid, name: lname, totalScore: 0 };
        learningScores[lid].totalScore += entry.score;
      }
    }
    ...
    for (const entry of fullResult) {
      if (entry.score !== null && entry.learningId?._id) {
        const exists = user.learningDailyHistory.some(
          h => h.learningId.toString() === entry.learningId._id.toString() && h.date === entry.date && h.session === user.session
        );
        if (!exists) {
          user.learningDailyHistory.push({
            learningId: entry.learningId._id,
...
exports.StrikePath = async (req, res) => {
  try {
    ...
    // Use updatedAt as start date, just like scoreCard
    const todayStr = moment().format('YYYY-MM-DD');
    const datesList = Array.from(scoreMap.keys()).sort();
    const minDate = user.updatedAt ? moment(user.updatedAt).startOf('day') : (datesList.length ? moment(datesList[0]) : moment().startOf('day'));
    const maxDate = datesList.length ? moment(datesList[datesList.length - 1]) : moment().startOf('day');
