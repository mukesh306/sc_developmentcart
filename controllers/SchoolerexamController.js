...
exports.UsersExams = async (req, res) => {
  try {
    ...
    // ------------------------------------------------------------------
    // *** NEW RULE ADDED BY YOUR REQUIREMENT ***
    // If first exam attend = true and result = failed
    // → all next exams: attend = false, isEligible = false
    // ------------------------------------------------------------------
    if (visibleExams.length > 0) {
      const firstExam = visibleExams[0];

      if (firstExam.attend === true && firstExam.result === "failed") {
