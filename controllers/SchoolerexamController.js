  ...
  try {
    ...
    for (const exam of exams) {
      ...
      if (existingStatus && moment(examDay).isBefore(today)) {
        updatedExams.push({
          ...examObj,
          statusManage: existingStatus.statusManage,
          result: existingStatus.result,
          rank: existingStatus.rank,
          correct: existingStatus.correct,
          finalScore: existingStatus.finalScore,
          percentage: existingStatus.percentage,
          totalParticipants: existingStatus.totalParticipants,
          attemptStatus: existingStatus.attemptStatus,
          publish: existingStatus.publish ?? exam.publish,
          className: existingStatus.className ?? examObj.className,
          totalQuestions:
            existingStatus.totalQuestions ??
            exam.topicQuestions?.length ??
            0,
        });
        continue;
      ...
      if (statusManage === "Completed") {
        ...
        examObj.percentage =
          userResult && examObj.totalQuestions > 0
            ? parseFloat(
                ((userResult.finalScore / examObj.totalQuestions) * 100).toFixed(
                  2
                )
              )
            : null;
