...
exports.platformDetails = async (req, res) => {
  try {
    const userId = req.params.id; 
    const requestedLevel = parseInt(req.query.level || 0);
    const { endDate } = req.query; // Pass YYYY-MM-DD format

    if (!userId || !endDate) {
      return res.status(400).json({ message: 'userId and endDate are required.' });
    }

    // Find user with matching endDate in active User collection
    let user = await User.findOne({ _id: userId, endDate: new Date(endDate) }).lean();

    // If not found, check in UserHistory
    if (!user) {
      user = await UserHistory.findOne({ originalUserId: userId, endDate: new Date(endDate) }).lean();
    }

    if (!user) {
      return res.status(404).json({ message: 'User not found for given endDate.' });
    }
    ...
    const startDate = moment(datesList[0]);
    const endMoment = moment(datesList[datesList.length - 1]);
    const result = [];
    ...
    for (let m = moment(startDate); m.diff(endMoment, 'days') <= 0; m.add(1, 'days')) {
      const currentDate = m.format('YYYY-MM-DD');
      const isToday = currentDate === moment().format('YYYY-MM-DD');
      const item = { date: currentDate, data: [] };
