...
cron.schedule("*/1 * * * *", async () => {
  try {
    ...
    const allUsers = await User.find().select("_id token");
    ...
    for (const user of allUsers) {
      ...
      try {
        await axios.get("http://localhost:5000/api/v1/UsersExams", {
          headers: {
        ...
        console.log("Exam Request Sent to User:", user._id);

      } catch (err) {
        console.log("Error for user", user._id, err.message);
      }
    }
  } catch (error) {
