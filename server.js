...
setInterval(async () => {
  try {
    ...
    for (const exam of exams) {
      // get all statuses fresh everytime
      const freshStatuses = await ExamUserStatus.find({ examId: exam._id }).lean();

      // -------------------------------------------------------
      // 1️⃣  AUTO BLOCK — ANY PREVIOUS EXAM FAILED → NOT ELIGIBLE
      // -------------------------------------------------------
      let blockExam = false;

      for (const user of freshStatuses) {
        const prevFail = await ExamUserStatus.findOne({
          userId: user.userId,
          examId: { $ne: exam._id },
          result: "Failed",
        }).lean();
        ...
        if (prevFail) {
          ...
          await ExamUserStatus.updateMany(
            { examId: exam._id, userId: user.userId },
            { statusManage: "Not Eligible", result: "Not Eligible" }
          );
        }
      }
      ...
      // update DB status (but not override result)
      await ExamUserStatus.updateMany(
        { examId: exam._id },
        { $set: { statusManage } }
      );
      ...
      // -------------------------------------------------------
      // 3️⃣   RESULT DECISION (REAL DB VALUES)
      // -------------------------------------------------------
      const updatedStatuses = await ExamUserStatus.find({ examId: exam._id }).lean();
      const anyAttempt = updatedStatuses.some(
        (u) => u.finalScore !== null && u.finalScore !== undefined
      );

      let finalResult = updatedStatuses[0]?.result || null;

      // Only calculate on Completed
      if (statusManage === "Completed") {
        if (!finalResult) {
          finalResult = anyAttempt ? "Completed" : "Not Attempt";
      ...
      // -------------------------------------------------------
      // 4️⃣   PUSH REAL-TIME DATA TO SOCKET
      // -------------------------------------------------------
      socketArray.push({
        examId: exam._id,
        statusManage: updatedStatuses[0]?.statusManage || statusManage,
        ScheduleDate: exam.ScheduleDate,
        ScheduleTime: exam.ScheduleTime,
        bufferTime,
        updatedScheduleTime: ongoingStart.format("HH:mm:ss"),
        result: updatedStatuses[0]?.result || finalResult,
        users: updatedStatuses,
      });
    }
